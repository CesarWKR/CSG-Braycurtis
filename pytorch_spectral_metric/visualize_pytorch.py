...
def make_graph(difference, title: str, classes: List[str]):
    """
    Plot the graph of `ds_name`
    Args:
        difference: 1 - D matrix computed (can be torch.Tensor or np.ndarray)
        title: str: name of dataset
        classes: Class names.
    """
    if isinstance(difference, torch.Tensor):
        difference = difference.cpu().numpy()  # Convertir tensor de PyTorch a numpy
