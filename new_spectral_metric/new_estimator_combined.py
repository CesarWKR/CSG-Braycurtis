...
class SimilarityCalculator:  
    ...
    def bray_curtis_similarity_blocked(self, data, block_size=200):  
        data_torch = torch.tensor(data, device=self.device, dtype=torch.float32)  
        num_samples = data_torch.size(0)  
        similarity_matrix = torch.zeros((num_samples, num_samples), device='cpu', dtype=torch.float32)  
    ...
    def cosine_similarity_blocked(self, data, block_size=200):  
        data_torch = torch.tensor(data, device=self.device, dtype=torch.float32)  
        data_torch = torch.nn.functional.normalize(data_torch, p=2, dim=1)  
        num_samples = data_torch.size(0)  
        similarity_matrix = torch.zeros((num_samples, num_samples), device='cpu', dtype=torch.float32)  
        ...
        for start_i in range(0, num_samples, block_size):  
            ...
            for start_j in range(0, num_samples, block_size):  
                ...
                similarity_block = torch.mm(block_i, block_j.t())  
                similarity_matrix[start_i:end_i, start_j:end_j] = similarity_block.cpu()  

        return similarity_matrix.numpy()  

    def combined_similarity_blocked(self, data, block_size=200, method='sum'):  
        bray_curtis = self.bray_curtis_similarity_blocked(data, block_size)  
        cosine_similarity = self.cosine_similarity_blocked(data, block_size)  

        if method == 'sum':  
            combined_similarity = bray_curtis + cosine_similarity  
        elif method == 'multiply':  
            combined_similarity = bray_curtis * cosine_similarity  
        else:  
            raise ValueError("Method should be either 'sum' or 'multiply'")  
        
        return combined_similarity  

    def compute_similarity_matrix(self, data, block_size=200, method='combined'):  
        if method == 'braycurtis':  
            return self.bray_curtis_similarity_blocked(data, block_size=block_size)  
        elif method == 'cosine':  
            return self.cosine_similarity_blocked(data, block_size=block_size)  
        elif method == 'combined':  
            return self.combined_similarity_blocked(data, block_size=block_size)  
        else:  
            raise ValueError("Method should be either 'braycurtis', 'cosine', or 'combined'")  

    def compute_intra_class_similarity(self, data, class_indices, block_size=200, method='combined'):  
        similarities = {}  
        for i, indices in class_indices.items():  
            class_data = data[indices]  
            similarities[i] = self.compute_similarity_matrix(class_data, block_size=block_size, method=method)  
        return similarities  
...
class CumulativeGradientEstimator:  
    ...
    def compute(self, data, target, class_samples):  
        ...
        self.C = self.similarity_calculator.compute_intra_class_similarity(data, self.class_indices, method='combined')  
