...
class SimilarityCalculator:  
    ...
    def bray_curtis_similarity_blocked(self, data, block_size=256):  # Adjust block size for better performance  
        data_torch = torch.tensor(data, device=self.device, dtype=torch.float32)  # Use float32 for better compatibility  
        num_samples = data_torch.size(0)  
    ...
    def compute_intra_class_similarity(self, data, class_indices, block_size=256):  # Adjust block size  
        similarities = {}  
        for i, indices in class_indices.items():  
            class_data = data[indices]  
            similarities[i] = self.bray_curtis_similarity_blocked(class_data, block_size=block_size)  
        return similarities  
